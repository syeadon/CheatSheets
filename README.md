# CheatSheets

Write once read twice

## TOC

- [Bash](./docs/bash/bash.md)
- [Git Configuration](./docs/git/git-config.md)
- [Git Development](./docs/git/git-dev.md)
  - [Git Undo](./docs/git/git-undo.md)
- [Git Bisect](./docs/git/git-bisect.md)
- [Git Diff](./docs/git/git-diff.md)
- [Git Merge](./docs/git/git-merge.md)
- [Git Rebase](./docs/git/git-rebase.md)
- [Git Flow](./docs/git/git-flow.md)
- [Git Log](./docs/git/git-log.md)
- [Git Lens](https://git/github.com/eamodio/vscode-gitlens/)
- [Visual Studio Code](./docs/vscode/vscode.md)
- [Cmder Keyboard Shortcuts](./docs/cmder/cmder-keyboard-shortcuts.md)

